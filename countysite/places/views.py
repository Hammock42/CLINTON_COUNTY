...
def place_list(request):
    places_query = Resource.objects.filter(places_page=True)
...
def filtered_place_list(request):
    ...
    if selected_sort == 'az':
        places_query = Resource.objects.filter(places_page=True).order_by('name')
...
def place_detail(request, place_slug):
    place = Resource.objects.get(slug=place_slug)
    context = {
        'place': place
    }
